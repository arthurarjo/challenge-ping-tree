  ...
  redis.keys('targets:*', function (err, data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      redis.hgetall(element, function (err, target) {
        ...
        if (target.lastVisitTimestamp && lastVisitTimestamp.setHours(0, 0, 0, 0) === options.timestamp.setHours(0, 0, 0, 0) && target.maxAcceptsPerDay === target.visits) eligible = false
